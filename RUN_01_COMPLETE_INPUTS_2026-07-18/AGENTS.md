...
Build a minimal, reliable long-running software-engineering harness that is Codex-native, provider-portable at the adapter boundary, WSL2-first, repository-centered, resumable, bounded, independently verifiable and safe by default.
...
## Post-ADR validation targets

After the scaffold exists, use repository-defined commands. Expected categories are:
...
Do not claim these commands exist until manifests and scripts have been inspected.
...
- unrelated structure was preserved;
- state and handoff records were updated;
- remaining risks and `UNKNOWN`s are explicit;
...
Stop with `BLOCKED` or `APPROVAL_REQUIRED` when work needs:

- ambiguous repository identity or missing authoritative input;
- credentials or broader network/filesystem access;
- destructive or non-idempotent action;
- commit, push, PR, merge, release or deployment;
- external message, purchase or production mutation;
- acceptance-criteria expansion;
- repeated no-progress or exhausted budget.

A local commit requires explicit user authorization. Push, PR, merge, release, deployment and external mutation are not authorized by default.
