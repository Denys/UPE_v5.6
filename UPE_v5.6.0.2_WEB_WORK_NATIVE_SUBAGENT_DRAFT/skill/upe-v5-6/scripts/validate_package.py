#!/usr/bin/env python3
"""Validate the complete UPE v5.6.0.2 draft/release or extracted upe-v5-6 skill."""
...
RELEASE_REQUIRED = [
    ...
    "evals/web_work_native_subagent_cases.yaml",
    MANIFEST,
]
...
SKILL_REQUIRED = [
    ...
    "references/UPE_v5.6.0.2_FULL_REFERENCE.md",
    "references/UPE_v5.6.0.2_PORTABLE_KERNEL.md",
    "references/UPE_v5.6.0.2_PROJECT_INSTRUCTIONS_KERNEL.md",
    "references/WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md",
    "scripts/validate_package.py",
]

RELEASE_TO_SKILL_COPIES = {
    "01_UPE_v5.6.0.2_FULL_REFERENCE.md": "references/UPE_v5.6.0.2_FULL_REFERENCE.md",
    "02_UPE_v5.6.0.2_PROJECT_INSTRUCTIONS_KERNEL.md": (
        "references/UPE_v5.6.0.2_PROJECT_INSTRUCTIONS_KERNEL.md"
    ),
    "03_UPE_v5.6.0.2_PORTABLE_KERNEL.md": "references/UPE_v5.6.0.2_PORTABLE_KERNEL.md",
    ...
    "13_WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md": ("references/WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md"),
    "evals/acceptance_cases.yaml": "evals/acceptance_cases.yaml",
    "evals/terminal_audit_cases.yaml": "evals/terminal_audit_cases.yaml",
    "evals/web_work_native_subagent_cases.yaml": ("evals/web_work_native_subagent_cases.yaml"),
}
...
def validate_web_work_adapter(skill_root: Path, errors: list[str]) -> None:
    cases_path = skill_root / "evals/web_work_native_subagent_cases.yaml"
    adapter_path = skill_root / "references/WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md"
    try:
        suite = yaml.safe_load(cases_path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        fail(errors, f"invalid Web Work eval suite: {exc}")
        return

    if not isinstance(suite, dict):
        fail(errors, "Web Work eval suite is not a mapping")
        return
    if suite.get("adapter") != "web_work_native_subagent":
        fail(errors, "Web Work eval suite has wrong adapter id")
    if suite.get("framework_version") != "5.6.0.2":
        fail(errors, "Web Work eval suite has wrong framework version")
    if suite.get("execution_status") not in {"NOT_RUN", "PASS", "FAIL", "PARTIAL"}:
        fail(errors, "Web Work eval suite has invalid execution_status")

    cases = suite.get("cases")
    if not isinstance(cases, list):
        fail(errors, "Web Work eval suite has no cases list")
        return
    ids = [case.get("id") for case in cases if isinstance(case, dict)]
    expected_ids = [f"W{number:02d}" for number in range(1, 18)]
    if ids != expected_ids:
        fail(errors, f"Web Work eval ids/order mismatch: {ids}")
    for case in cases:
        if not isinstance(case, dict):
            fail(errors, "Web Work eval case is not a mapping")
            continue
        if not isinstance(case.get("fixture"), dict):
            fail(errors, f"Web Work eval {case.get('id')} lacks fixture mapping")
        if not isinstance(case.get("expected"), dict):
            fail(errors, f"Web Work eval {case.get('id')} lacks expected mapping")

    try:
        adapter = adapter_path.read_text(encoding="utf-8")
    except OSError as exc:
        fail(errors, f"could not read Web Work adapter: {exc}")
        return
    required_markers = [
        ...
        "inline_frozen_copy | immutable_reference | shared_writable_path",
        ...
        "For a formal-release target, continue from the published",
    ]
    for marker in required_markers:
        if marker not in adapter:
            fail(errors, f"Web Work adapter missing marker: {marker!r}")
    print(f"PASS web_work_eval_cases={len(cases)}")
...
def main() -> int:
    ...
    errors: list[str] = []
    release_mode = (root / "01_UPE_v5.6.0.2_FULL_REFERENCE.md").is_file()
    ...
    kernel = (
        root / "02_UPE_v5.6.0.2_PROJECT_INSTRUCTIONS_KERNEL.md"
        if release_mode
        else root / "references/UPE_v5.6.0.2_PROJECT_INSTRUCTIONS_KERNEL.md"
    )
    portable = (
        root / "03_UPE_v5.6.0.2_PORTABLE_KERNEL.md"
        if release_mode
        else root / "references/UPE_v5.6.0.2_PORTABLE_KERNEL.md"
    )
    full_reference = (
        root / "01_UPE_v5.6.0.2_FULL_REFERENCE.md"
        if release_mode
        else root / "references/UPE_v5.6.0.2_FULL_REFERENCE.md"
    )
    worker = (
    ...
    if all((skill_root / path).is_file() for path in SKILL_REQUIRED):
        ...
        validate_web_work_adapter(skill_root, errors)
