# UPE v5.6.0.2 — GPT-5.6 Runtime Profile

**Availability profile date:** 2026-07-26
**Framework revision date:** 2026-07-26
...
- final `.2`: second compatible patch on that line;
- stable core lineage: inherited from UPE v4.1 and the v4.3 adaptive-branching branch;
- current framework draft: v5.6.0.2;
- frozen published baseline: v5.6.0.1;
- future compatible patches on this line: v5.6.0.3 and so on;
...
v5.6.0.1 added the terminal-audit core invariant CC-5.6.0.1-01. v5.6.0.2 adds the compatible `web_work_native_subagent` runtime adapter without weakening that invariant.

## 2. Keep seven axes separate
...
7. **Publication mode:** standard hidden-v1 review, fast visible unaudited draft, or formal release.
...
- Current Work documentation describes subagent-backed operation, and Ultra uses subagents for suitable tasks. Actual explicit spawn/result/context controls remain capability-dependent and must be verified in the current run.
...
| Web Work native subagent | Serial hidden-v1 audit/rework when fresh-child spawn, result return, and candidate-copy controls are exposed | Assuming plan/model labels prove operations; passing only a shared writable draft; claiming security isolation without evidence |
...
Use for the serial terminal-review loop on Web Work when current-run capability evidence passes. It is not a parallel adapter. Standard mode keeps v1 hidden, surfaces Reviewer 1’s validated output, lets UPE publish v2, and sends v2 to a new Reviewer 2. Fast mode publishes v1 as `DRAFT_UNAUDITED` and cannot bypass formal release review.
...
## 9. Web Work native-subagent profile

Select `web_work_native_subagent` only when the current run exposes:

- fresh child creation without parent conversation history;
- child result return to the coordinator;
- lifecycle/status or equivalent wait behavior;
- candidate transport as an inline frozen copy or immutable reference.

Native child threads can provide cognitive independence while inheriting host capabilities and permissions. Record:

- cognitive independence;
- candidate integrity/transport;
- action authority;
- technical isolation and tool-allowlist evidence.

Do not claim a per-child sandbox or security-grade isolation from the word “subagent.” Shared writable-path review is advisory unless an independent enforcement mechanism closes the gap.

Immediate review uses native spawn/result operations and hash-bound envelopes. Hooks or heartbeats may support lifecycle/liveness on another surface, but they do not transport review prose or establish independence. Use `13_WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md` for the complete state machine, exit thresholds, fast mode, steering, and recovery.

## 10. Recommended UPE routing shorthand
...
- **Web Work native subagent:** `fresh child operations + hidden v1 + visible review + UPE v2 + bounded post-review`.
