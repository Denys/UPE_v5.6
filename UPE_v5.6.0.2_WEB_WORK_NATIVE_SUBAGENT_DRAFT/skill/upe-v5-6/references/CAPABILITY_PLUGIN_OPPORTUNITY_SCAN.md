# UPE v5.6.0.2 — Capability, Skill, Plugin, Connector, and Tool Opportunity Scan
...
- material creation or revision of a reusable framework that requires the terminal independent auditor/improver gate.

It is optional and usually omitted for trivial one-off writing, translation, or simple Q&A.

For this gate, `material` means an observable change to behavior, authority, required inputs/outputs, capability routing, action boundaries, state/recovery, deployment, or acceptance. Semantic-preserving mechanical edits are non-triggers.
...
8. **Independent review route:** Fresh delegated worker, `web_work_native_subagent`, fresh serial process/chat, external reviewer handoff, or unavailable. Check actual operations, context, candidate transport, authority, and isolation—not merely the label “subagent.”
9. **Fallback:** Define a no-capability or degraded route that preserves critical correctness and safety. Same-context review may advise but cannot satisfy an independent gate.
...
| Terminal reusable-framework review | Fresh independent read-only worker | Frozen candidate/hash, no candidate authorship, motivated findings, baseline score, projected delta, complete revision, coordinator acceptance |
| Immediate Work terminal loop | Web Work native subagent | Fresh-child/result operations, hidden v1, one visible/unchanged review payload, UPE v2, fresh post-review, bounded exit |
...
capability_plan:
  ...
  independent_review:
    ...
    evidence:
      ...
      technical_isolation: proven | not_proven | unknown
    fallback_handoff:
  residual_limits:
```
...
- Do not call same-context self-critique independent. A qualifying reviewer must not have authored the candidate, must receive a frozen evidence bundle, and must be read-only.
- Select `web_work_native_subagent` from verified current-run operations, not plan/model documentation alone. It is serial delegation, not native parallelism.
- Keep cognitive independence, candidate integrity, action authority, and technical isolation separate. Never infer a sandbox or reviewer tool allowlist from the host label.
- In Work standard mode keep v1 hidden, surface and reuse one validated review payload, publish UPE’s v2, then use a new fresh post-reviewer. Fast mode may expose `DRAFT_UNAUDITED` v1 but cannot bypass a formal release gate.
- If no independent route exists, prepare the handoff and keep the gate `UNKNOWN`; do not silently downgrade the requirement.
...
- For a reviewer: did it author or mutate the candidate, receive hidden author reasoning, or share mutable draft state? Any “yes” disqualifies independence.
- For a Work child: can the parent prove fresh context, result return, exact candidate hash, non-writable candidate transport, bounded lifecycle, and no external-action authority? If not, downgrade the affected claim.
...
- for terminal framework review, its independence proof, frozen-bundle interface, quantified audit schema, revision authority, and coordinator integration boundary.
