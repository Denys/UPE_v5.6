...
2. Read `references/UPE_v5.6.0.2_FULL_REFERENCE.md` for serious evaluation, architecture, migration, framework creation/revision, or conflict resolution.
...
6. Read `references/WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md` when Web Work exposes fresh-child delegation and the terminal gate or explicit reviewer loop applies.
7. Use `scripts/validate_package.py` for deterministic release or skill validation and inspect `evals/` for behavioral cases.
...
9. On Web Work, select `web_work_native_subagent` only from current-run spawn/result/context evidence. Standard mode hides v1, surfaces Reviewer 1’s validated payload unchanged, lets UPE publish v2, then uses a fresh Reviewer 2 and bounded exits. Fast mode publishes `DRAFT_UNAUDITED` v1 but cannot bypass formal release review.
10. Integrate only evidence-backed changes, record one disposition per proposed change, and rerun affected checks.
11. Deliver verified scope or an exact partial result.
...
For Web Work, distinguish cognitive independence from technical isolation. A fresh child must receive an inline frozen copy or immutable reference without author history. Shared writable-path review is advisory unless enforcement closes the gap; never infer a security sandbox or tool allowlist from “subagent.”
...
For the complete draft/release:
