# UPE v5.6.0.2 — Independent Framework Auditor and Improver
...
For `web_work_native_subagent`, a fresh child must receive an inline frozen candidate copy or immutable reference, not the original writable draft or parent history. Report cognitive independence separately from technical isolation. A shared writable-path review is advisory and leaves the independent gate `UNKNOWN` unless the host proves an enforcement mechanism. The parent/child state machine, visible review handoff, fast mode, hash recheck, and convergence rules are in `13_WEB_WORK_NATIVE_SUBAGENT_ADAPTER.md`.
...
6. use one audit cycle normally and stop after a maximum of two, except the Web Work adapter’s explicitly bounded pre/post-review state machine;
