# UPE v5.6.1 — Official Source Map
...
**v5.6.1 scope note:** The terminal independent auditor/improver contract is a UPE design requirement introduced by `CORE_CHANGE` CC-5.6.1-01, not a claim that every OpenAI surface exposes an independent worker. OAI-07 supports the availability and suitable shape of multi-agent work where exposed; each deployment must still verify actual context separation, authorship, permissions, and read-only scope.
