# UPE v5.6.0.1 Release Pack

UPE v5.6.0.1 adds a terminal independent framework-audit and improvement gate while preserving the GPT-5.6 runtime target:

```text
v5.6.0.1 = GPT-5.6 runtime adapter + UPE line 0 + compatible patch 1
```

`CORE_CHANGE CC-5.6.0.1-01` requires a materially created or revised reusable framework to be frozen and reviewed by a qualifying independent, read-only worker before release. The worker must produce motivated findings, an evidence-anchored 0–50 baseline, improvement headroom, a calibrated projected delta, any empirical delta separately, a complete proposed revision, and a reasoned version decision.

## Start here

1. Paste `02_UPE_v5.6.0.1_PROJECT_INSTRUCTIONS_KERNEL.md` into Project Instructions.
...
The candidate remains frozen after the authorized relabel to `5.6.0.1`. The `5.6.1` identifier is reserved and blocked from Codex use. Do not rename, promote, or generate a `5.6.1` release without explicit user authorization.

## Contents

| File | Purpose |
|---|---|
| `01_UPE_v5.6.0.1_FULL_REFERENCE.md` | Durable manual, exact audit schema, scoring, delta, version, and integration contracts |
| `02_UPE_v5.6.0.1_PROJECT_INSTRUCTIONS_KERNEL.md` | Always-on kernel, hard-capped below 8,000 characters |
| `03_UPE_v5.6.0.1_PORTABLE_KERNEL.md` | Reduced cross-project/lower-tier kernel |
...
| `07_CHANGELOG_AND_MIGRATION.md` | v5.6.0 → v5.6.0.1 change and migration record |
