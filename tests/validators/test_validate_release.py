...
def test_valid_package_manifest_and_archive_pass() -> None:
    with tempfile.TemporaryDirectory(prefix="c306-release-", dir=ROOT / ".pytest_cache") as tmp:
        package, manifest, archive = make_package(Path(tmp))
...
def test_hash_mismatch_fails_nonzero_with_expected_and_actual_digest() -> None:
    with tempfile.TemporaryDirectory(prefix="c306-release-", dir=ROOT / ".pytest_cache") as tmp:
        package, manifest, _ = make_package(Path(tmp))
...
def test_unlisted_package_file_fails_nonzero() -> None:
    with tempfile.TemporaryDirectory(prefix="c306-release-", dir=ROOT / ".pytest_cache") as tmp:
        package, manifest, _ = make_package(Path(tmp))
...
def test_unsafe_manifest_path_fails_nonzero() -> None:
    with tempfile.TemporaryDirectory(prefix="c306-release-", dir=ROOT / ".pytest_cache") as tmp:
        package, manifest, _ = make_package(Path(tmp))
        manifest.write_text(
...
def test_archive_traversal_member_fails_without_extraction() -> None:
    with tempfile.TemporaryDirectory(prefix="c306-release-", dir=ROOT / ".pytest_cache") as tmp:
        package, manifest, archive = make_package(Path(tmp))
